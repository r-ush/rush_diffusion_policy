#!/usr/bin/env python
"""
온라인 residual DAgger Actor — INTERVENTION(physical-push) 판. (박스삽입, 오른팔+손, wrench)

residual_teleop_actor_env_runner.py(teleop 판)와 추론/relabel/전송/hot-swap 골격은 같지만,
**개입 방식이 다르다**:

  teleop 판:   페달로 제어를 servo(팔)+manus(손)에 넘겨 사람이 팔+손을 모두 몬다.
  intervention 판(이 파일): 제어를 넘기지 않는다.
    * 팔:  base(+residual)가 임피던스 컨트롤러(/right_dsr_controller/task_space_command)로
           **계속** 나가고, 사람이 그 팔을 **물리적으로 민다**(임피던스라 밀림).
    * 손:  개입 중에도 **항상 base(slow) 가 자율 제어** (manus 안 씀).
    * 페달('a'=개입시작 / 'b'=개입종료 토글): 제어를 안 넘기고, 어느 프레임이 "개입"인지
           per-step label(is_intervention)만 표시. learner 가 그 프레임을 가중 학습한다.

왜 이게 깨끗한가:
  relabel 수식은 teleop 판과 동일 — residual_target = achieved(t+1) − slow_pred(t).
  base 가 매 tick forward 를 이미 명령하므로, 안 밀 때는 achieved≈base → residual≈0,
  밀 때만 residual=밀림(사람 교정). 즉 base 의 전진은 상쇄되고 사람 밀림만 잔차로 남는다.
  ★ 이 정렬을 위해 **개입 중에는 매 tick slow 를 새로 예측**한다(fresh base). 개입 중 base 를
    멈추면(명령 정지) residual 이 "전진 취소 + 밀림" 으로 오염되므로 반드시 계속 실행한다.

온라인 훅 3개(teleop 판과 동일):
  1. head hot-swap (에피소드 경계): head+normalizer(+force_encoder)만, slow 불변.
  2. slow_pred 로깅 (매 tick): 그 스텝의 base(slow 예측 abs pose9). 실행값이 아니라 slow-only.
  3. relabel+전송 (유지 시): write_residual_episode_hdf5(is_intervention 포함) → mailbox.

★★★ 로봇/카메라/servo 없이 실행 검증 불가(하드웨어 필요). 로봇 앞에서 [VERIFY] 지점 실측.
    servo/manus 는 **불필요**(핸드오프 안 함). 임피던스 stiffness 는 사람이 밀 수 있을 만큼
    낮게(로봇 설정). 상세: online_learning/RESIDUAL_INTERVENTION_README.md

실행 (2 터미널: learner + 이 actor):
  RESIDUAL_INTERVENTION_WORKDIR=data/online_runs/run_hand_intervention \
  RESIDUAL_INTERVENTION_SLOW_CKPT=/media/rush/.../260714_insert_box_hand_rel/epoch=0500-train_loss=0.002.ckpt \
  /home/rush/anaconda3/envs/bae_robodiff/bin/python \
    online_learning/residual_intervention_actor_env_runner.py --use_hand \
...
# 키 입력 헬퍼만 재사용 (개입형은 TeleopControlPub/servo 핸드오프를 쓰지 않는다)
...
def _fast_head_ready(fast_policy):
    """learner 가 1회 이상 학습해 normalizer 를 채워 발행했는지. 빈 normalizer 로 predict 하면
    _build_head_input 의 normalize 가 죽으므로, 그때는 residual 을 적용하지 않고 slow-only 로
    돌며 교정만 모은다(교정 0개일 때 residual=0 은 DAgger 부트스트랩상 올바른 동작)."""
    try:
...
@click.command()
@click.option('--input', '-i', default=None, help='slow(base) ckpt. 미지정 시 config_residual_intervention.SLOW_CKPT.')
...
@click.option('--steps_per_inference', '-si', default=6, type=int, help='(nominal) slow chunk 재계획 전 실행할 스텝 수.')
...
def main(input, output, config_name, robot_ip, steps_per_inference, max_duration,
    ...
    def cap_residual(residual6):
        """[VERIFY] residual delta6 캡 — 방향 유지, 크기만 상한(초기 발산 안전). 사람 밀림(물리)은
        이 캡의 대상이 아니다(캡은 head 출력만 제한)."""
        ...
        with DualarmRealEnv(
            ...
            def process_key(key):
                ...
                if key in ('a', 'A'):
                    if not intervening:
                        intervening = True
                        print("[Actor] ▶ 개입 시작 (밀어서 교정). base 계속 실행 + 이 구간 label=1.")
                elif key in ('b', 'B'):
                    if intervening:
                        intervening = False
                ...
                while True:
                    ...
                    if not head_ready:
                        print("[Actor] 학습된 residual head 아직 없음 → 이번 에피소드는 slow-only 실행 + "
                              "교정 데이터만 수집. (a=개입 → s=전송, "
                              f"{C.MIN_EPISODES_BEFORE_TRAIN}개 이상 쌓이면 learner 학습→발행)")
                    ...
                    intervening = False
                    episode_done = None
                    frames = []                     # per-tick 프레임(RAW_OBS + slow_pred + is_intervention)
                    ...
                    while True:
                        ...
                        # ── slow chunk 재계획 ──
                        #   nominal: chunk 소진 시에만(부드러운 궤적, slow 연산 amortize).
                        #   개입 중: 매 tick 신선한 slow 예측을 base 로 → residual = achieved(다음)
                        #     − 신선한 slow = 순수 밀림(stale drift 없음). (개입은 사람 속도라 tick 당
                        #     slow 추론 ~0.2s 여도 무방.)
                        usable = 0 if slow_abs_seq is None else min(steps_per_inference, len(slow_abs_seq))
                        if slow_abs_seq is None or slow_step >= usable or intervening:
                            with torch.no_grad():
                        ...
                        # ── 실행: nominal·개입 모두 항상 base(+residual) 을 임피던스로 명령 ──
                        #   개입 중에도 팔은 계속 base 를 실행(사람이 그 위에 물리적으로 민다).
                        #   손(slow_hand)은 항상 base 자율. 제어를 넘기지 않으므로 pause/resume 없음.
                        action_ts = obs_ts[-1] + dt
                        env.exec_actions(
                            actions=final_action[None],
                            timestamps=np.array([action_ts]),
                            stages=np.array([1 if intervening else 0], dtype=np.int64))
                    ...
                    # ── residual 포맷으로 relabel + 전송 (is_intervention 포함) ──
