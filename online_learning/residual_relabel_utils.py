...
출력 HDF5 구조(FastResidualContextStepDataset + task/hand_online.yaml 이 읽는 포맷):
  data/<demo_name>/obs/{image0, robot_pose_R, robot_quat_R, hand_pose_R, wrench_wrist_R,
                        ...
                        [is_intervention, residual_pred6, residual_cmd6]}  # 개입 판에서만
...
def build_residual_demo(episode: dict):
    ...
    # (선택) head 가 그 tick 에 낸 값 — 라운드 2 이후 head 기여분과 사람 추가분을 사후에
    # 분리하려면 필요하다. residual_pred6 = 원 출력(e, 게인·캡 전), residual_cmd6 = 실제 얹은
    # 명령 delta(게인·캡 후). 마지막 스텝은 residual 과 동일하게 버린다.
    for _k in ("residual_pred6", "residual_cmd6"):
        if _k in episode:
            demo[_k] = np.asarray(episode[_k], dtype=np.float32)[:n]
