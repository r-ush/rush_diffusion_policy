...
class OnlineLearner:
    ...
    def _build_dataset(self):
        ...
        # 이 repo의 dataset이 받지 않는 cfg 키(예: 학습 repo에서 온
        # future_wrench_steps)는 걸러낸다. **kwargs가 없으면 TypeError가 나므로.
        sig = inspect.signature(cls.__init__)
        if not any(p.kind == inspect.Parameter.VAR_KEYWORD
                   for p in sig.parameters.values()):
            accepted = set(sig.parameters) - {"self"}
            dropped = [k for k in ds_cfg if k not in accepted]
            if dropped:
                print(f"[Learner] dataset이 받지 않는 cfg 키 무시: {dropped}")
