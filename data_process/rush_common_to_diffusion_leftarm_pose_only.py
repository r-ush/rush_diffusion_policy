...
""" diffusion data (pose only), 10Hz
data
    demo_0
        actions (robot_pose_L(3), robot_6d_L(6))  # 9D
        obs
...
def main():
    ...
    output_filename = '/media/rush/00d3eaaf-732e-4a7f-8bd3-6fee68d14fe7/260518_0136/diffusion_data_leftarm_pose_only20.hdf5'
    output_demo_idx = 0
        ...
        for input_filename in input_filenames:
            with h5py.File(input_filename, 'r') as input_file:
                ...
                for demo_idx in tqdm.tqdm(indices_to_process, desc=f"Processing {input_filename}"):
                    ...
                    # actions 저장: pose_L(3) + 6d_rotation_L(6) = 9D
                    output_6d_rotation_L = quat_to_6d(output_TCP_quat_L)
                    output_actions = np.hstack([output_TCP_pose_L, output_6d_rotation_L]).tolist()
