...
class DiffusionTransformerHybridImagePolicy(BaseImagePolicy):
    def __init__(self, 
        ...
        cond_dim = n_emb if obs_as_cond else 0 # True
        n_cond_tokens = n_obs_steps if obs_as_cond else None

        # AdaLN DiT consumes condition tokens in transformer hidden dim.
        self.obs_cond_proj = nn.Identity()
        if obs_as_cond and obs_feature_dim != n_emb:
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:   
        ...
        if self.obs_as_cond:   # obs를 Transformer에 condition으로 따로줌
            ...
            cond = self.obs_cond_proj(nobs_features.reshape(B, To, -1))
            shape = (B, T, Da)
    ...
    def get_optimizer(
        ...
        obs_params.extend(self.obs_cond_proj.parameters())
        optim_groups.append({
            "params": obs_params,
    ...
    def compute_loss(self, batch):
        ...
        if self.obs_as_cond:
            ...
            cond = self.obs_cond_proj(nobs_features.reshape(batch_size, To, -1))
