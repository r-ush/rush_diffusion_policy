...
class _FlowSchedulerStub:
    """Minimal scheduler-like object for old num_train_timesteps plumbing."""
...
class FlowMatchingUnetHybridImagePolicy(BaseImagePolicy):
    def __init__(self, 
        ...
        if noise_scheduler is None:
            noise_scheduler = _FlowSchedulerStub(
                num_train_timesteps=max(1, int(flow_time_scale))
            )
        self.flow_time_scale = float(flow_time_scale)
        self.flow_t_min = float(flow_t_min)
        self.flow_t_max = float(flow_t_max)
        if not (0.0 <= self.flow_t_min < self.flow_t_max <= 1.0):
            raise ValueError(
                "Flow time bounds must satisfy 0 <= flow_t_min < flow_t_max <= 1."
            )
        ...
        print(
            "Flow matching sampler: steps=%d, time_scale=%g, t_range=[%g, %g]"
            % (
    ...
    def conditional_sample(self, 
        ...
        if n_steps <= 0:
            raise ValueError(f"num_inference_steps must be positive, got {n_steps}")
        ...
        for step_idx in range(n_steps):
            ...
            # 2. predict flow velocity
            t = time_grid[step_idx].expand(batch_size)
            dt = time_grid[step_idx + 1] - time_grid[step_idx]
            model_output = model(trajectory, self._model_time(t), 
                local_cond=local_cond, global_cond=global_cond)

            # 3. Euler integrate: x_t -> x_{t+dt}
    ...
    def compute_loss(self, batch):
        ...
        # Sample noise endpoint x_0 and time t for rectified flow.
        ...
        timesteps = torch.empty(
            ...
            device=trajectory.device,
            dtype=trajectory.dtype,
        ).uniform_(self.flow_t_min, self.flow_t_max)
        t_view = timesteps.reshape((bsz,) + (1,) * (trajectory.ndim - 1))
        ...
        # Predict the flow velocity v = x_1 - x_0
        pred = self.model(noisy_trajectory, self._model_time(timesteps), 
            local_cond=local_cond, global_cond=global_cond)
